 ...
 * method in either the request, or the request body (using 'http method
 * spoofing', ie adding a hidden `_method` field to a form) and calls the
 * handler that corresponds to that method.
 *
 * The `methods` parameter is an object where each key is callable, named
 * after a http method (eg get, post, put, delete) and gets `req` and `res`
 * as parameters.
 *
 * {
 *   get(req, res) {
...
function handleByMethod(methods = {}) {
	return async (req, res) => {
		const method = (req.body && req.body._method || req.method).toLowerCase()
